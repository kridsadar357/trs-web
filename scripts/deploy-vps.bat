...
REM .next/standalone/public breaks /site.webmanifest, /logo.png, /hero/*, etc. (404 on localhost:3000).
REM Use -o BatchMode=yes only when SSH key auth works (no password prompt).
ssh %VPS_USER%@%VPS_HOST% "cd %VPS_DIR% && git pull origin %GIT_BRANCH% && npm ci && npm exec prisma generate && npm run build && mkdir -p .next/standalone/public .next/standalone/.next/static .next/standalone/trs-web/public .next/standalone/trs-web/.next/static && rm -rf .next/standalone/public/* .next/standalone/.next/static/* .next/standalone/trs-web/public/* .next/standalone/trs-web/.next/static/* && cp -r public/. .next/standalone/public/ && cp -r .next/static/. .next/standalone/.next/static/ && cp -r public/. .next/standalone/trs-web/public/ && cp -r .next/static/. .next/standalone/trs-web/.next/static/ && pm2 restart trs-next"
